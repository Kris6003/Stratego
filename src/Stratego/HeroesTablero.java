package Stratego;


import java.util.Arrays;

/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */

/**
 *
 * @author jenniferbueso
 */
public class HeroesTablero {
    
   // Matriz que almacena las posiciones de los héroes
    private int[][] Posicion_Heroes;

    // Constructor de la clase
    public HeroesTablero() {
        // Inicialización de la matriz y contador de héroes
        Posicion_Heroes = new int[4][10];
        int contheroes = 1;

        // Bucle para llenar la matriz con números del 1 al 40
        for (int i = 0; i < Posicion_Heroes.length; i++) {
            for (int j = 0; j < Posicion_Heroes[i].length; j++) {
                Posicion_Heroes[i][j] = contheroes++;
            }
        }

        // Llamada al método para imprimir la matriz
        imprimirMatriz();
    }

    // Método privado para imprimir la matriz de manera formateada
    private void imprimirMatriz() {
        for (int i = 0; i < Posicion_Heroes.length; i++) {
            System.out.println(Arrays.toString(Posicion_Heroes[i]));
        }
    }

    // Método getter para obtener la matriz de posiciones de héroes
    public int[][] getPosicion_Heroes() {
        return Posicion_Heroes;
    }

    // Método main para probar la funcionalidad de la clase
    public static void main(String[] args) {
        new HeroesTablero();
    } 
    
}
